...
| 3 | TID and CID lookups return structured rows and extensibility metadata. | In progress | Python resolvers now return structured TID and CID rows with extensibility metadata; CLI exposure exists for TID, CID, and code lookup, while MCP exposure remains pending before completion. |
...
| Branch | main |
| Last completed commit | Pending current commit; previous completed commit was cba86d9. |
| Last verification | `uv run --dev pytest tests/unit/test_cli_lookup.py -k code` passed with 1 passed and 30 deselected; `uv run --dev pytest tests/unit/test_cli_lookup.py` passed with 31 passed; sandboxed `make lint` failed before running because `uv` could not read `/Users/beatrice/.cache/uv/sdists-v9/.git`; escalated `make lint` passed; escalated `make typecheck` passed; escalated `make test` passed with 288 passed and 4 skipped. |
| Current blocker | None |
| Commit-ready summary | Added the CLI `dicom-kb lookup code <code-value> [--scheme <scheme>]` command for the existing Python `lookup_code_meaning` resolver, with focused CLI coverage using the synthetic PS3.16 fixture. |
| Next recommended action | Continue Phase 5 with the next smallest public exposure slice: add the MCP `dicom_lookup_sr_template` tool for the existing Python resolver; do not add the context-group/code MCP tools or legal/docs updates in that slice. |
...
| Pending current commit | Added the CLI `lookup code` command for the existing Python resolver, with focused CLI coverage against the synthetic PS3.16 fixture. | `uv run --dev pytest tests/unit/test_cli_lookup.py -k code`; `uv run --dev pytest tests/unit/test_cli_lookup.py`; `make lint`; `make typecheck`; `make test` |
